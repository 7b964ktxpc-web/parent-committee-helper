...
function scenarioRules(text) {
  ...
  if (t.includes('ответь родителю')) return 'ОТВЕТ РОДИТЕЛЮ: отвечай именно на мысль родителя. Используй только факты, которые есть в текущем запросе или в контексте группы. Не пересказывай её, не начинай автоматически с «понимаю», «спасибо», «извините», «конечно». Нельзя выдумывать причины суммы, решения родкома, согласования, обсуждения, расходы, покупки, договорённости или будущие действия. Нельзя обещать «уточню», «узнаю», «напишу позже», «скину список», если пользователь этого не просил и не дал соответствующую информацию. Если в исходных данных сказано только, что нужно собрать 500 рублей на подарок, нельзя превращать это в «утверждено родкомом», «сумма сложилась из расходов» или другую причину. Если хватает одной фразы — одна фраза.';
...
- ИСТОЧНИК ФАКТОВ: факты можно брать только из текущего запроса пользователя и явно указанного контекста группы. Предыдущие ответы ассистента — это черновики, а не факты. Никогда не переноси из них утверждения о том, что было решено, утверждено, куплено, согласовано или обещано.
- Если факта нет, не заполняй пробел фантазией. Лучше написать нейтрально или оставить информацию без пояснения.
