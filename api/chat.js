...
function scenarioRules(text) {
  ...
  if (t.includes('ответь родителю')) return 'ОТВЕТ РОДИТЕЛЮ: напиши именно сообщение, которое пользователь может отправить родителю. Для этого режима используй только текущий запрос пользователя и явно сохранённый контекст группы. Не используй предыдущие сообщения пользователя как источник фактов. Не объясняй пользователю, какие данные тебе известны или неизвестны. Не пиши «в исходных данных не указано», «информации нет», «я не знаю». Если причина чего-то не дана, просто НЕ называй причину. Не выдумывай причины суммы, решения родкома, согласования, обсуждения, расходы, покупки, договорённости и будущие действия. НЕЛЬЗЯ вставлять квадратные скобки или подсказки. НЕЛЬЗЯ писать «могу уточнить», «уточню», «узнаю», «пришлю список», «напишу позже», если это прямо не попросил пользователь. Если родитель считает сумму большой, можно спокойно предложить обсудить меньшую сумму — без новых фактов. Обычно 1–2 короткие фразы.';
...
function extractCurrentAmount(text) {
  const source = String(text || '');
  return source.match(/(?:по|сдать|сдавать|собрать|сумм[аеы]?)[^\d]{0,30}(\d{2,6})\s*(?:руб(?:лей|ля|ль)?|₽)?/i)?.[1]
    || source.match(/(\d{2,6})\s*(?:руб(?:лей|ля|ль)?|₽)/i)?.[1]
    || '';
}

function cleanReply(text, replyMode, currentUserText = '') {
  ...
  // Ловим незаполненную цель в любом месте ответа, а не только в самом конце.
  // Например: «Сумма 500 рублей рассчитана на . Если хотите...».
  const brokenTarget = /(?:рассчитан[ао]?|предназначен[ао]?|ид[её]т)\s+на\s*[.!?](?:\s|$)/i.test(result)
    || /\bна\s*[.!?](?:\s|$)/i.test(result);
  if (brokenTarget) {
    const amount = extractCurrentAmount(currentUserText);
    if (amount) return `Если считаете, что ${amount} рублей — это много, можем обсудить сумму поменьше.`;
    return 'Если считаете, что сумма большая, можем обсудить вариант поменьше.';
  }

  // Убираем неподтверждённые условия о «большинстве».
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // В режиме ответа родителю прошлые сообщения вообще не отправляем модели.
    // Иначе она может принять старую цель сбора денег за факт текущего сообщения.
    const replyHistory = replyMode ? [{ role: 'user', content: lastUser }] : userMessages;
    ...
    try {
      ...
      if (model.endsWith('/aliceai-llm-flash')) {
        ...
        return res.json({ reply: cleanReply(reply, replyMode, lastUser), model: 'aliceai-llm' });
      }
      throw firstError;
    }

    return res.json({ reply: cleanReply(reply, replyMode, lastUser), model: model.split('/').pop() });
  } catch (error) {
