...
function scenarioRules(text) {
  ...
  if (t.includes('ответь родителю')) return 'ОТВЕТ РОДИТЕЛЮ: отвечай именно на мысль родителя. Используй только факты из текущего запроса или контекста группы. Предыдущие ответы Кати вообще не являются источником фактов. НЕЛЬЗЯ дописывать причины суммы, решения родкома, согласования, обсуждения, расходы, покупки, договорённости и будущие действия. НЕЛЬЗЯ вставлять квадратные скобки или подсказки вроде «[указать...]». НЕЛЬЗЯ писать «могу уточнить», «уточню», «узнаю», «пришлю список», «напишу позже», если это прямо не попросил пользователь. Если родитель спрашивает, почему нужна сумма, а причина не дана, прямо скажи, что причина в исходных данных не указана. Не пытайся заполнить этот пробел. Если можно ответить одной-двумя фразами — ответь одной-двумя фразами.';
...
Не нужно автоматически защищать родком, соглашаться с родителем или искать компромисс. Ответь на конкретную мысль человека. Спокойный прямой ответ лучше длинной дипломатии. Если причина или решение не указаны пользователем — честно обозначь отсутствие этой информации, не выдумывай объяснение.
...
function cleanReply(text, replyMode) {
  ...
  // Модель иногда всё ещё пытается оставить шаблонные заполнители или обещания.
