...
function scenarioRules(text) {
  ...
  if (t.includes('ответь родителю')) return 'ОТВЕТ РОДИТЕЛЮ: отвечай именно на мысль родителя. Используй только факты, которые есть в текущем запросе или в контексте группы. Предыдущие ответы Кати не являются источником фактов и не должны использоваться как фактическая основа. Не пересказывай её, не начинай автоматически с «понимаю», «спасибо», «извините», «конечно». Нельзя выдумывать причины суммы, решения родкома, согласования, обсуждения, расходы, покупки, договорённости или будущие действия. Нельзя обещать «уточню», «узнаю», «напишу позже», «скину список», если пользователь этого не просил. Если исходные данные не объясняют, почему установлена сумма, не придумывай объяснение. Отвечай только тем, что можно подтвердить из данных. Если хватает одной фразы — одна фраза.';
...
- Если факта нет, не заполняй пробел фантазией. Лучше написать нейтрально или прямо сказать только то, что известно.
- Не превращай предположение в факт. Слова «предложили», «утвердили», «решили», «договорились», «собрали», «купили», «уточню» требуют прямого основания в данных пользователя.
...
Не нужно автоматически защищать родком, соглашаться с родителем или искать компромисс. Ответь на конкретную мысль человека. Спокойный прямой ответ лучше длинной дипломатии. Если причина или решение не указаны пользователем — не придумывай их.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // В задачах «ответь родителю» прошлые ответы модели полностью исключаем.
    // Иначе модель может принять собственную прошлую выдумку за факт и повторить её.
    const replyHistory = isReplyToParent(lastUser)
    ...
    let reply;
    const temperature = isReplyToParent(lastUser) ? 0.45 : 0.9;
    try {
