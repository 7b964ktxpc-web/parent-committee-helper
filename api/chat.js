  ...
  try {
    ...
    try {
      ...
      if (model.endsWith('/aliceai-llm-flash')) {
        ...
        return res.json({ reply: cleanReply(reply, replyMode, replyHistory), model: 'aliceai-llm' });
      }
      throw firstError;
    }

    return res.json({ reply: cleanReply(reply, replyMode, replyHistory), model: model.split('/').pop() });
  } catch (error) {
